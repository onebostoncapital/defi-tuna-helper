...
# Initialize state to prevent "vanishing"
if 'cached_signal' not in st.session_state:
    st.session_state.cached_signal = {
        "bias": "ANALYZING...", "lev": "CALCULATING...", "conf": 0, "clr": "#333"
    }
...
# 2. THE BACKGROUND ENGINE (Sync every 66s)
now = time.time()
if (now - st.session_state.last_sync) > 66:
    ...
    for t in tfs:
        ...
        if ok:
            ...
            if p > s and p > e: l_votes += 1
            elif p < s and p < e: s_votes += 1
            
    conf = max(l_votes, s_votes)
    if conf >= 4:
        side = "GO LONG" if l_votes >= s_votes else "GO SHORT"
        levs = {4: "2x", 5: "3x", 6: "4x", 7: "5x", 8: "5x"}
        st.session_state.cached_signal = {
            "bias": side,
            "lev": f"USE {levs.get(conf, '2x')} LEVERAGE",
            "conf": conf,
            "clr": "#0ff0" if "LONG" in side else "#f44"
        }
    else:
        st.session_state.cached_signal = {
            "bias": "NEUTRAL", "lev": "WAIT FOR CONSENSUS", "conf": conf, "clr": "#555"
        }
    st.session_state.last_sync = now

# 3. UI RENDERING (Uses the Locked Session State)
...
# 4. GLOBAL BIAS BOX (THE "NON-VANISHING" BLOCK)
# We pull directly from session_state so it is never empty during the 3-second refresh
sig = st.session_state.cached_signal
remaining = max(0, int(66 - (time.time() - st.session_state.last_sync)))

st.markdown("---")
st.markdown(
    f"""
    <div style="background-color: #1e2129; border: 5px solid {sig['clr']}; border-radius: 15px; padding: 50px; text-align: center;">
        ...
        <h1 style="color: white; font-size: 85px; margin: 10px 0; font-weight: 900;">
            GLOBAL BIAS: <span style="color: {sig['clr']};">{sig['bias']}</span>
        </h1>
        <h2 style="color: {sig['clr']}; font-size: 55px; margin: 5px 0; font-weight: 700;">
            {sig['lev']}
        </h2>
        <p style="color: #666; font-size: 16px; margin-top: 25px; border-top: 1px solid #333; padding-top: 15px;">
            Confidence: {sig['conf']}/8 Judges | Method: EMA 20 + SMA 200 | Next Sync: {remaining}s
        </p>
    </div>
...
for i, t in enumerate(t_list):
    if nav[i].button(t, key=f"tf_{t}", use_container_width=True):
