...
with st.expander("🔐 Sentinel & Drift Wallet Status"):
    ...
    auto_pilot = st.toggle("🚀 ENABLE AUTO-PILOT (Drift Execution)")
    
    if "SOLANA_PRIVATE_KEY" in st.secrets:
        st.success("✅ Drift Wallet Connected via Secrets")
    else:
        st.error("⚠️ Private Key missing in Settings > Secrets")
...
if status:
    ...
    consensus_count = max(longs, shorts)
    conv = "STRONG" if consensus_count >= 6 else "MODERATE"
    c3.metric("Consensus", f"{consensus_count}/8 Alignment", conv)
    ...
    # 5. WAR ROOM & AUTO-EXECUTION
    st.markdown("---")
    st.subheader("✍️ War Room: Drift Deployment")
    ...
    if auto_pilot and conv == "STRONG":
        ...
        # Logic: If price hits target and we haven't traded in the last 5 mins
        if (side == "LONG" and price >= entry_req) or (side == "SHORT" and price <= entry_req):
            if time.time() - st.session_state.last_trade_time > 300:
                try:
                    # DRIFT CALL (MOCKED FOR SAFETY UNTIL FINAL CONFIRMATION)
                    st.warning(f"🚀 AUTO-EXECUTING DRIFT {side} AT ${price}")
                    
                    # EMAIL LOG
                    content = f"AUTO-TRADE TRIGGERED\nSide: {side}\nPrice: {price}\nJudges: {consensus_count}/8"
                    msg = MIMEText(content); msg['Subject'] = "⚡ Drift Auto-Pilot Alert"; msg['From'] = sender; msg['To'] = sender
                    with smtplib.SMTP_SSL('smtp.gmail.com', 465) as s:
                        s.login(sender, pwd); s.send_message(msg)
                    
                    st.session_state.last_trade_time = time.time()
                    st.success("Trade complete. Sentinel cooling down for 5 minutes.")
                except Exception as e: st.error(f"Auto-Pilot Error: {e}")

    # Manual Backup
    if st.button("🚀 Manual Drift Trade Now"):
        st.info("Sending Manual Order to Drift...")
