...
def fetch_base_data(interval="1h"):
    try:
        # Mapping for the 8 specific judges defined in our rule-set
        period_map = {
        ...
        # 2. Standardize columns to lowercase to prevent KeyError
        df = df.reset_index()
        df.rename(columns={df.columns[0]: 'date'}, inplace=True)
        df.columns = [str(c).lower() for c in df.columns]

        # 3. Indicator Calculation (Rules: 20 EMA, 200 SMA)
        df['20_ema'] = df['close'].ewm(span=20, adjust=False).mean()
        df['200_sma'] = df['close'].rolling(window=200).mean()
        
        # 4. Fetch BTC-USD for Header
